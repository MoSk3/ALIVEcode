function editModeSection(s) {
    ...
    s.enterEditMode = () => {
        ...
        //********************************* Menus **********************************
        // Top menu
        s.topMenu = s.spawnFixedRect(0, s.height / 2 - (s.isMobile ? 18.75 : 25), s.width, (s.isMobile ? 37.5 : 50), 600)
        ...
        // Side menu
        s.sideMenu = s.spawnFixedRect(-s.width / 2 + (s.isMobile ? 75 : 100), -s.topMenu.getHeight() / 2, (s.isMobile ? 150 : 200), s.height - s.topMenu.getHeight())
        ...
        s.sideMenu.onScroll((e) => {
            // Limite pour scroll dans la barre de menu
            const limiteScroll = 25

            if (
                (s.sideMenu.children[0].top.y + limiteScroll > s.sideMenu.top.y && e.delta < 0)
                ||
                (s.sideMenu.children[s.sideMenu.children.length - 1].bottom.y - limiteScroll < -s.height / 2 && e.delta > 0)) {
                for (let i = 0; i < s.sideMenu.children.length; i++) {
                    const posNew = new Vector(s.sideMenu.children[i].pos.x, s.sideMenu.children[i].pos.y + e.delta / 2)
    ...
    s.spawnSideMenuItems = () => {
        ...
        s.sideMenu.children.forEach((child) => {
            if (!(child instanceof FixedTextObject)) {
                let image = s.spawnFixedRect(child.left.x + child.getWidth() / ratio / 2, child.top.y - child.getHeight() / ratio / 2, child.getWidth() / ratio, child.getHeight() / ratio)
                child.resizeShape(image)
                s.deleteShape(image)

                child.setPos(new Vector(s.sideMenu.pos.x + (count % 3 * (size + space)) - (size + space), top - size / 2), false)
                count += 1
                ...
                // position des textes
                child.setPos(new Vector(s.sideMenu.pos.x - s.sideMenu.getWidth() / 2 + space * 1.5 + child.getWidth() / 2, top + size / 2 - space), false)
            }
        })
    }
    ...
    s.menuRoadItems = () => {
        ...
        tire_tracks1.setTexture(tire_tracks, 0.72)
    ...
    s.menuTerrainItems = () => {
        ...
        gazon1.setTexture(grass, 2 / 3 * 0.5)
        gazon1.onClickDown(() => {
            if (!s.topMenu.hovering)
        ...
        mars_terrain1.setTexture(mars_terrain, 0.3)
        ...
        dark_lava1.setTexture(darkLava, 0.3)
    ...
    s.menuObstacleItems = () => {
        ...
        maison1.setImg(house)
        maison1.setSoundOnCollision(house_impact_audio)
        maison1.onClickDown(() => {
            if (!s.topMenu.hovering)
        ...
        stone1.setImg(stone)
        stone1.setSoundOnCollision(stone_impact_audio)
        stone1.onClickDown(() => {
            if (!s.topMenu.hovering)
        ...
        rocheMars1.setImg(marsRock1)
        rocheMars1.setSoundOnCollision(stone_impact_audio)
        ...
        rocheMars2.setImg(marsRock2)
        rocheMars2.setSoundOnCollision(stone_impact_audio)
        ...
        mountains1.setImg(mountains)
        mountains1.setSoundOnCollision(stone_impact_audio)
        ...
        tree1.setImg(tree)
        tree1.setSoundOnCollision(tree_impact_audio)
        tree1.onClickDown(() => {
            if (!s.topMenu.hovering)
        ...
        water1.setTexture(water, 0.3)
        water1.setSoundOnCollision(water_impact_audio)
        ...
        lava1.setTexture(lava, 0.3)
        lava1.setSoundOnCollision(lava_bubbles_audio)
        lava1.gameOverEvent = {
        ...
        hole1.setImg(hole)
        hole1.setSoundOnCollision(falling_down_audio)
        hole1.gameOverEvent = {
        ...
        mars_dome1.setImg(mars_dome)
        mars_dome1.setSoundOnCollision(metal_impact_audio)
        ...
        broken_rocket1.setImg(broken_rocket)
        broken_rocket1.setSoundOnCollision(metal_impact_audio)
        broken_rocket1.onClickDown(() => {
            if (!s.topMenu.hovering)
                s.spawnShapeFromMenu(broken_rocket1, s.spawnObstacle(150, 150, false, 4))
        })
        ...
        black_hole1.setImg(black_hole)
        black_hole1.setSoundOnCollision(falling_down_audio)
    ...
    s.menuInteractiveObjectItems = () => {
        ...
        coinObject.setImg(coin)
        coinObject.setSoundOnCollision(coin_collect_audio)
        coinObject.onClickDown(() => {
            if (!s.topMenu.hovering)
                s.spawnShapeFromMenu(coinObject, s.spawnInteractiveObject(75, 100, true, false, false, 5))
        })
        ...
        repair_tools1.setImg(repair_tools)
        repair_tools1.setSoundOnCollision(coin_collect_audio)
        repair_tools1.onClickDown(() => {
            if (!s.topMenu.hovering)
                s.spawnShapeFromMenu(repair_tools1, s.spawnInteractiveObject(100, 100, true, false, false, 5))
        })
        ...
        finishLineObject.setTexture(finishLine, 2 / 3 * 0.5)
        finishLineObject.setSoundOnCollision(level_complete_audio)
        finishLineObject.onClickDown(() => {
            if (!s.topMenu.hovering)
                s.spawnShapeFromMenu(finishLineObject, s.spawnInteractiveObject(195, 75, false, true, false, 5))
        })
        ...
        flag1.setImg(flag)
        flag1.setSoundOnCollision(level_complete_audio)
        flag1.onClickDown(() => {
            if (!s.topMenu.hovering)
                s.spawnShapeFromMenu(flag1, s.spawnInteractiveObject(150, 150, false, true, false, 5))
        })
        ...
        button_red_green.setImg(red_button)
        //button_red_green.setSoundOnCollision(level_complete_audio)
        button_red_green.onClickDown(() => {
            if (!s.topMenu.hovering)
                s.spawnShapeFromMenu(button_red_green, s.spawnInteractiveObject(100, 100, false, false, true, 5))
        })
        ...
        treadmill1.setTexture(treadmill, 0.2)
        ...
        spring1.setImg(spring)
        spring1.setSoundOnCollision(coin_collect_audio)
        spring1.onClickDown(() => {
            if (!s.topMenu.hovering)
                s.spawnShapeFromMenu(spring1, s.spawnInteractiveObject(75, 100, false, false, false, 5))
        })
    ...
    s.menuOtherShapeItems = () => {
        // Figure fleche
        const fleche = s.spawnFixedObject(500, [-75, 25], [30, 25], [30, 50], [75, 0], [30, -50], [30, -25], [-75, -25])
        fleche.color = 'blue'
        fleche.onClickDown(() => {
            if (!s.topMenu.hovering)
                s.spawnShapeFromMenu(fleche, s.spawnShape(10, [-75, 25], [30, 25], [30, 50], [75, 0], [30, -50], [30, -25], [-75, -25]))
        })
        ...
        triangle.onClickDown(() => {
            if (!s.topMenu.hovering)
                s.spawnShapeFromMenu(triangle, s.spawnShape(10, [-75, -150], [0, -50], [75, -150]))
        })
    ...
    s.spawnShapeFromMenu = (menuItem, shape, copiedShaped = false) => {
        ...
        //if (shape.isRect()) {
            if (menuItem.hasTexture)
                shape.setTexture(menuItem.img, menuItem.res)
            else
                shape.setImg(menuItem.img)
        //}
        ...
        shape.onClickDown(() => {
            if (s.editMode) {
                if (shape.pinned)
                    s.pinButton.setImg(pinOn)
                else
                    s.pinButton.setImg(pinOff)
            }
        })
        ...
        if(shape.isGameOver)
            shape.setGameOverEvent(menuItem.gameOverEvent)

        if (!copiedShaped) {
            s.pressedObject = shape
            s.pressedObject.mouseOffset = s.mouse.clone().substract(s.pressedObject.pos)
        }
    }
    ...
    s.spawnTopMenuItems = () => {
        ...
        s.topMenu.children.forEach((child) => {
    ...
    s.spawnUndoButton = () => {
        // Undo button
        s.undoButton = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.undoButton.setImg(undo)
                    ...
                    if (lastShapeModified.wereDeletedViaTrash) {
                        ...
                        */
                        ...
                        for (let i = 0; i < lastShapeModified.shapeData.length; i++) {
                            ...
                            tableauClone.push(s.storeShapeData(lastShapeModified.shapeData[i].originalShape, false, false, true))
                            s.revertToLastShape(lastShapeModified.shapeData[i])
                            s.addShapeWithExtraSteps(lastShapeModified.shapeData[i].originalShape)
                        }

                        s.redoShapes.push({ shapeData: tableauClone, isMultipleObjects: true, wereDeletedViaTrash: true, wasDeletedViaRedo: false })
                        s.undoShapes.pop()
                    }
                    else {
                        /*
                        ...
                        */
                        ...
                        // Pour chaque shape dans ShapeData
                        for (let i = 0; i < lastShapeModified.shapeData.length; i++) {
                            tableauClone.push(s.storeShapeData(lastShapeModified.shapeData[i].originalShape))
                            s.revertToLastShape(lastShapeModified.shapeData[i])
                        }
                            ...
                            if (s.undoShapes[i].isMultipleObjects) {
                                for (let j = 0; j < s.undoShapes[i].shapeData.length; j++) {
                                    for (let y = 0; y < lastShapeModified.shapeData.length; y++) {
                                        if (s.undoShapes[i].shapeData[j].originalShape.id == lastShapeModified.shapeData[y].originalShape.id) {
                                ...
                                for (let y = 0; y < lastShapeModified.shapeData.length; y++) {
                                    if (s.undoShapes[i].originalShape.id == lastShapeModified.shapeData[y].originalShape.id) {
                                        hasPreviousShape = true
                                    }
                                }
                            }
                        ...
                        if (hasPreviousShape == false) {
                            ...
                            for (let i = 0; i < lastShapeModified.shapeData.length; i++) {
                                ...
                                s.deleteShapeWithExtraSteps(lastShapeModified.shapeData[i].originalShape)

                            }
                            s.redoShapes.push({ shapeData: tableauClone, isMultipleObjects: true, wereDeletedViaTrash: false, wereDeletedViaUndo: true, wasDeletedViaRedo: false })
                        }
                        // Sinon, s'il Y A un shape antérieur
                        else {
                            s.redoShapes.push({ shapeData: tableauClone, isMultipleObjects: true, wereDeletedViaTrash: false, wereDeletedViaUndo: false, wasDeletedViaRedo: false })
                        }
                ...
                // S'il y a eu une modification sur UN SEUL shape
                    ...
                    for (let i = s.undoShapes.length - 1; i >= 0; i--) {
                        if (s.undoShapes[i].isMultipleObjects != true) {
                            ...
                            if (s.undoShapes[i].originalShape.id == lastShapeModified.originalShape.id) {

                                hasPreviousShape = true

                                // Si le dernier shape modifier n'a pas été effacé via Undo ou via TrashCan
                                if (lastShapeModified.wasDeletedViaUndo == false && lastShapeModified.wasDeletedViaTrash == false) {
                                    s.redoShapes.push(s.storeShapeData(lastShapeModified.originalShape))
                                }
                                else {
                                    ...
                                    if (lastShapeModified.wasDeletedViaUndo) {
                                        ...
                                        s.redoShapes.push(s.storeShapeData(lastShapeModified.originalShape, true))
                                    }
                                    // Si dernier shape modifier à été effacer via le bouton TrashCan
                                        ...
                                        s.redoShapes.push(s.storeShapeData(lastShapeModified.originalShape, false, false, true))
                                    }
                    ...
                    if (hasPreviousShape == false) {
                        ...
                        for (let car of s.cars)
                            if (car.shape == lastShapeModified.originalShape)
                                isCar = true
    ...
    s.spawnRedoButton = () => {
        // Bouton Redo
        s.redoButton = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.redoButton.setImg(redo)
            ...
            if (s.redoShapes.length > 0) {
                ...
                if (lastShapeModified.isMultipleObjects) {
                    if (lastShapeModified.wereDeletedViaTrash) {
                        ...
                        s.undoShapes.push({ shapeData: tableauClone, isMultipleObjects: true, wereDeletedViaTrash: false, wereDeletedViaUndo: false, wasDeletedViaRedo: false })
                        s.redoShapes.pop()
                    }
                    // Si la collection de shapes dernierement modifier n'avaient pas été effacés via TrashCan ou via Undo
                        ...
                        s.undoShapes.push({ shapeData: tableauClone, isMultipleObjects: true, wereDeletedViaTrash: false, wasDeletedViaRedo: false })
                        s.redoShapes.pop()
                    }
                }
                // S'il y a eu une modification sur UN SEUL shape
                    ...
                    if (lastShapeModified.wasDeletedViaTrash) {
                        s.addSingleUndoWithData(lastShapeModified.originalShape, false, false, true)
                        s.deleteShapeWithExtraSteps(lastShapeModified.originalShape)
                        s.redoShapes.pop()
                        return
                    }
                    ...
                    for (let i = s.redoShapes.length - 1; i >= 0; i--) {
                        ...
                        if (s.redoShapes[i].originalShape.id == lastShapeModified.originalShape.id) {
                            ...
                            if (lastShapeModified.wasDeletedViaUndo == false && lastShapeModified.wasDeletedViaTrash == false) {
                                ...
                                if (lastShapeModified.wasDeletedViaUndo) {
                                    s.addSingleUndoWithData(lastShapeModified.originalShape, true)
                                    s.addShapeWithExtraSteps(lastShapeModified.originalShape)
                                }
                            }
                            // Revient au changements précendents
    ...
    s.spawnCopyButton = () => {
        ...
        s.copyButton = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.copyButton.setImg(copy)
        s.copyButton.onClick(() => {
            ...
            // Si la collection de shapes sélectionnés contient plus que 1 shape
                ...
                s.lastCopiedObjects = []
                s.listPreviousSelect.forEach((shape) => {
    ...
    s.spawnPasteButton = () => {
        s.pasteButton = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.pasteButton.setImg(pasteOff)
    ...
    s.spawnRotateLeftButton = () => {
        s.rotate_Counter_Clockwise = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.rotate_Counter_Clockwise.setImg(rotate_Counter_Clockwise_Img)
    ...
    s.spawnRotateRightButton = () => {
        s.rotate_Clockwise = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.rotate_Clockwise.setImg(rotate_Clockwise_Img)
    ...
    s.spawnLinkButton = () => {
        s.linkButton = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.linkButton.setImg(linkOff)
    ...
    s.spawnPinButton = () => {
        s.pinButton = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.pinButton.setImg(pinOff)
        s.pinButton.onClick(() => {
            s.togglePinButton()
        })
    ...
    s.spawnSelectionButton = () => {
        s.selectionButton = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.selectionButton.setImg(selectionOff)
    ...
    s.spawnInformationButton = () => {
        s.informationButton = s.spawnFixedRect(0, 0, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.informationButton.setImg(information)
    ...
    s.spawnTrashCanButton = () => {
        s.trashCan = s.spawnFixedRect(s.width / 2 - 35, s.height / 2 - s.topMenu.getHeight() - 50, 50, 50)
        s.trashCan.setImg(trash)
    ...
    s.spawnZoomInButton = () => {
        s.zoom_In_Button = s.spawnFixedRect(-s.width / 2 + s.sideMenu.getWidth() + s.topMenuItemSize, -s.height / 2 + s.topMenuItemSize, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.zoom_In_Button.color = s.color(0, 0)
        s.zoom_In_Button.setImg(zoom_In)
    ...
    s.spawnZoomOutButton = () => {
        s.zoom_Out_Button = s.spawnFixedRect(-s.width / 2 + s.sideMenu.getWidth() + s.topMenuItemSize * 2 + 10, -s.height / 2 + s.topMenuItemSize, s.topMenuItemSize, s.topMenuItemSize, 650)
        s.zoom_Out_Button.color = s.color(0, 0)
        s.zoom_Out_Button.setImg(zoom_Out)
    ...
    s.removeInteractiveObject = (shape) => {
        if (shape instanceof InteractiveObject) {
            s.interactiveObjects.forEach((interactiveObject, index) => {
    ...
    s.addSingleUndo = (shape) => {
        s.undoShapes.push(shape)
    }

    //s.storeShapeData = (shape, wasDeletedViaUndo = false, wasRedo = false, wasDeletedViaTrash = false)
    s.addSingleUndoWithData = (shape, wasDeletedViaUndo = false, wasRedo = false, wasDeletedViaTrash = false) => {
        s.undoShapes.push(s.storeShapeData(shape, wasDeletedViaUndo, wasRedo, wasDeletedViaTrash))
    }
    ...
    */
    ...
    s.revertToLastShape = (shapeData) => {
        // Remet à sa position antérieur
        shapeData.originalShape.setPos(new Vector(shapeData.clonedShape.pos.x, shapeData.clonedShape.pos.y), false)
    ...
    s.pasteShape = () => {
        if (s.lastCopiedObjects.length != 0) {
            ...
            if (s.lastCopiedObjects.length > 1) {
                ...
                s.lastCopiedObjects.forEach(copiedShape => {
                    let shapeToPaste = s.pasteSingleShape(copiedShape)
                    shapeToPaste.setPos(s.mouse.clone().substract(copiedShape.shapeOffset).roundVector(1), false)
    ...
    s.rotateShapeFromMenu = (angle) => {
        ...
        if (s.listPreviousSelect.length > 0) {
            ...
            s.movableShapes.forEach(movableShape => {
                s.listPreviousSelect.forEach(shapeInSelect => {
                        ...
                        for (let i = s.undoShapes.length - 1; i >= 0; i--) {
                            if (!s.undoShapes[i].isMultipleObjects) {
                                if (s.undoShapes[i].originalShape.id == shapeInSelect.id) {
                                    let shapeClone = s.storeShapeData(s.undoShapes[i].originalShape)
                                ...
                                for (let j = 0; j < s.undoShapes[i].shapeData.length; j++) {
                                    if (s.undoShapes[i].shapeData[j].originalShape.id == shapeInSelect.id) {
                                        let shapeClone = s.storeShapeData(s.undoShapes[i].shapeData[j].originalShape)
                                        tableauClone.push(shapeClone)
                                    }
                                }
                            }
    ...
    s.contextMenuClick = () => {
        ...
        if (isHoveringShape && !s.contextMenuState) {
            ...
            for (const car of s.cars)
                if (car.shape == shapeHovering) isCar = true

            if (!isCar)
                s.toggleContextMenuShapeOn(shapeHovering)

            s.undoShapes.pop()
        }
        else if (!isHoveringShape && !s.contextMenuState && s.lastCopiedObjects.length != 0)
            s.toggleContextMenuCanvasOn()
        else if (s.contextMenuState)
            s.toggleContextMenuOff()
    }

    s.toggleContextMenuShapeOn = (shape) => {
        ...
        s.contextMenu = s.spawnRect(shape.calcRightX() + 100, shape.calcTopY() - 50, 200, shape.isButton ? 190 : 140, 400)
        s.contextMenu.color = '#303030'
        s.contextMenu.isHelp = true
        s.contextMenu.scaleWithCamera = true

        contextMenuPin = s.spawnTextObject(!shape.pinned ? "Pin" : "Unpin", 26, 0, 50, 400)
        ...
        if (shape.isButton) {
            contextMenuLink = s.spawnTextObject(s.linkButtonState ? "Cancel Linking" : "Add/Remove link", 26, 0, -100, 400)
        ...
        s.contextMenu.children.forEach(child => {
            child.color = 'white'
        ...
        contextMenuCopy.onClick(() => {
            if (s.listPreviousSelect.length != totalShapesInsideSelection)
                s.listPreviousSelect.splice(s.listPreviousSelect.length - totalShapesInsideSelection, totalShapesInsideSelection)
        ...
        contextMenuDelete.onClick(() => {
            ...
            if (s.lastMovableObject != null) {
                ...
                for (let selection of s.listPreviousSelect) {
                    ...
                    lastSelectedShapes.push(s.storeShapeData(selection, false, false, true))
                ...
                s.undoShapes.push({ shapeData: lastSelectedShapes, isMultipleObjects: true, wereDeletedViaTrash: true, wasDeletedViaRedo: false })
                s.undoShapes.push({ shapeData: lastSelectedShapes, isMultipleObjects: true, wereDeletedViaTrash: true, wasDeletedViaRedo: false })
    ...
    s.toggleSelectionButton = () => {
        if (s.selectionButtonState) {
            ...
            s.selectionButton.setImg(selectionOn)
    ...
    s.selectionClickDown = () => {
        ...
        if (s.selectionShape != null && !(s.keyIsDown(32) || s.keyIsDown(17)) && s.selectionShape.listSelect != null) {
            ...
            if (!inSelection) {
                if (!s.rotate_Counter_Clockwise.hovering || s.rotate_Clockwise.hovering) {
                ...
                if (s.selectionShape.listSelect.length > 0) {
                    let lastSelectedModifications = []

                    s.selectionShape.listSelect.forEach(shapeSelected => {
                        lastSelectedModifications.push(s.storeShapeData(shapeSelected))
                    });
                    s.undoShapes.push({ shapeData: lastSelectedModifications, isMultipleObjects: true, wereDeletedViaTrash: false, wasDeletedViaRedo: false })
                }
            }
        }
    }
    ...
    s.createSelection = () => {
        if ((s.pressedObject == null || s.pressedObject?.pinned === true) && s.startingPointSelection != null && s.selectionShape == null && s.editMode) {
    ...
    s.adjustSelection = () => {
        if (s.selectionShape != null) {
            if (s.selectionShape.colliding.length == 0 && !s.contextMenu.hovering) { //Suppression 
                s.deleteSelection()
            }
            else if (!s.selectionShape.movable) { //Ajustement de la forme
                const listLeftX = []
                const listRightX = []
                const listTopY = []
                const listBottomY = []
                const listSelect = []
                for (const collider of s.selectionShape.colliding.filter(shape => !shape.pinned)) {
            ...
            else if (s.trashCan.hovering || s.sideMenu.hovering && s.selectionShape.movable) { // Suppression de toute la selection
                let lastSelectedShapes = []
            ...
            if (s.selectionShape != null) {
                ...
                for (let selection of s.selectionShape.listSelect) {
                    selection.shapeOffset = s.selectionShape.pos.clone().substract(selection.pos)
    ...
    s.addOrRemoveLink = () => {
        ...
        for (let i = 0; i < s.movableShapes.length; i++) {
            if (s.movableShapes[i].originalShape.hovering && s.movableShapes[i].originalShape instanceof Obstacle) {
                // vérifie si l'objet selectionné à déjà un link avec le dernier bouton                             
                for (let j = 0; j < s.lastSelectedButton.linkedId.length; j++) {
                    if (s.lastSelectedButton.linkedId[j] == s.movableShapes[i].originalShape.id) {
                        objectAlreadyLinked = true
                        s.lastSelectedButton.linkedId.splice(j, 1)
                        break
                    }
                }
                if (objectAlreadyLinked)
                    s.showPopUpMessage('Link has been removed!')
                else {
                    s.lastSelectedButton.linkedId.push(s.movableShapes[i].originalShape.id)
                    s.showPopUpMessage('Link has been added!')
                    break
                }
            }
            else if (s.movableShapes[i].originalShape.hovering && !(s.movableShapes[i].originalShape instanceof Obstacle)) {
    ...
    s.togglePinButton = () => {
        if (s.lastPinnabledObject != null) {
            ...
            if (s.lastPinnabledObject.pinned) {
                ...
                s.pinButton.setImg(pinOff)
            s.showPopUpMessage(s.lastPinnabledObject.pinned ? 'Pinned!' : 'Unpinned!')
        }
    }

    s.addShapeWithExtraSteps = (shape) => {
    ...
    s.showPopUpMessage = (message, time = 1) => {
        ...
        if (s.popUpMessage == null) {
            s.popUpMessage = s.spawnFixedTextObject(message, 25, 0, s.height / 2 - s.topMenu.getHeight() * 2)
    ...
    s.spawnCustomSelection = (shape, color, gap) => {
        // Fait apparaitre un contour sur la shape qu'on veut
        let customSelection = s.spawnRect(shape.pos.x, shape.pos.y, shape.getWidth() + gap, shape.getHeight() + gap, 150)
