class Shape {
	...
	draw() {
		...
		if (this.hasTexture) {
			...
			for (let vertex of this.vertices) {
				if (vertex == null || vertex == NaN) alert("NULL")
				this.s.vertex(
	...
	getOrientationVector() {
		const angle = this.rotation.x > 270 ? this.rotation.x - 90 : (360 - (this.rotation.x - 90)) % 360
	...
	spawnRotateBox(image) {
		...
		let rotation = this.s.spawnRect(
			-image.getWidth() / 2 -
				this.s.editBoxSize / (1000 / this.camera.scale),
			image.getHeight() / 2 +
				this.s.editBoxSize / (1000 / this.camera.scale),
			this.s.editBoxSize,
			this.s.editBoxSize,
	...
	resizeShape(image) {
		...
		for (let vertex of this.vertices)
			this.resizeVectorToImage(vertex, image)
