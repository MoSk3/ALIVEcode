...
const useCmd = (): [ref: RefObject<HTMLDivElement>, cmd: CMD | null] => {
	...
	const print = (msg: string) => {
		...
		$cmd.append(
			`<span><u><i>${heures}:${minutes}:${secondes}:</i></u> ${msg}</span><br>`,
		);
	...
	const error = (msg: string, line: number) => {
		...
		msg = msg.replace(/</g, '&lt;').replace(/>/g, '&gt;');
		$cmd.append(
			`<span style="color: red"><u><i>${heures}:${minutes}:${secondes}:</i></u> Erreur à la ligne <strong>#${line} :<br></strong><i>"${msg}"</i></span><br>`,
		);
		$cmd.scrollTop(cmd.current.scrollHeight);
	};
