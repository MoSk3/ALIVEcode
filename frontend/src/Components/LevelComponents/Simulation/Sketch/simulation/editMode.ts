...
import { images, sounds, loadAllImages, unloadUnusedImages } from './assets';
...
export function editModeSection(s: any) {
	...
	s.exitEditMode = () => {
		...
		unloadUnusedImages(
			s,
			(Object.values(s.shapes) as Shape[]).map(shape => shape.img),
		);
	};
	...
	s.menuRoadItems = () => {
		...
		s.menuObstacleItems = () => {
			...
			rocheMars1.setImg(images.rocheMars1);
			...
			rocheMars2.setImg(images.rocheMars2);
			...
			broken_rocket1.setImg(images.brokenRocket);
		...
		s.menuInteractiveObjectItems = () => {
			...
			button_red_green.setImg(images.red_button_unpressed);
		...
		s.menuDecorationItems = () => {
			...
			crackHole1.setImg(images.holeCrack);
		...
		s.spawnShapeFromMenu = (
			...
			if (menuItem.hasTexture)
				shape.setTexture([menuItem.imgName, menuItem.img], menuItem.res);
			else shape.setImg([menuItem.imgName, menuItem.img]);
			//}
			...
			//if ((shape as Obstacle).isGameOver) (shape as Obstacle).setGameOverEvent(menuItem.gameOverEvent);
		...
		s.spawnRotateLeftButton = () => {
			...
			s.rotate_Counter_Clockwise.setImg(images.rotate_counterclockwise);
		...
		s.spawnRotateRightButton = () => {
			...
			s.rotate_Clockwise.setImg(images.rotate_clockwise);
		...
		s.addInteractiveObject = (shape: any) => {
			if (shape instanceof InteractiveObject) {
				...
				if (shape.isCollectable) s.coinsTotal += 1;
			}
		};
...
export const makeShapeEditable = (shape: Shape) => {
	const s = shape.s;
