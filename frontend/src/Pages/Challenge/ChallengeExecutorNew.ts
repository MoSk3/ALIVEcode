...
class ChallengeExecutor implements PlayExecutor {
  ...
  constructor(creator: User | undefined, levelName: string, playButton: HTMLButtonElement) {
  ...
  public init(s: any) {
    ...
    this.socket.onRobotReceive((data: any) => {
      ...
      this.d1?.moveInDirection(this.d1.forward?.normalize(), this.sensors.dG * 7, false);
      this.d2?.moveInDirection(this.d2.forward?.normalize(), this.sensors.dA * 7, false);
      this.d3?.moveInDirection(this.d3.forward?.normalize(), this.sensors.dD * 7, false);
    });
    ...
    this.playButton.on('click', (e: any) => {
      if (!this.execution) {
        ...
        this.socket?.compile(lines, (data) => {
          ...
          const perform_action = (i: number, res: Array<any> | undefined = undefined) => {
        ...
        this.playButton.children("img").attr("src", "/static/images/play_button.png");
  ...
  private spawnEditorButton() {
    let s = this.s;
    if(this.s) return;
    this.editorButton = s.spawnFixedRect(s.width / 2 - 35, -s.height / 2 + 95, 50, 50, 5000)
    // images.tools
  ...
  private spawnRobotConnectButton() {
    this.robotConnectButton = this.s.spawnFixedRect(this.s.width / 2 - 35, -this.s.height / 2 + 35, 50, 50, 1000)
