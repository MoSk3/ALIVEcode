class TextObject extends Shape {
    constructor(s, text, size, x, y, color = "black") {
        ...
        super(s, [(x - w / 2) * (s.canvasCamera.scale / 1000), (y + h / 2) * (s.canvasCamera.scale / 1000)], [(x + w / 2) * (s.canvasCamera.scale / 1000), (y + h / 2) * (s.canvasCamera.scale / 1000)], [(x + w / 2) * (s.canvasCamera.scale / 1000), (y - h / 2) * (s.canvasCamera.scale / 1000)], [(x - w / 2) * (s.canvasCamera.scale / 1000), (y - h / 2) * (s.canvasCamera.scale / 1000)])
    ...
    draw() {
        ...
        let translateX = (this.pos.x - this.camera.pos.x) * (1000 / this.camera.scale)
        let translateY = (this.camera.pos.y - this.pos.y) * (1000 / this.camera.scale)
        if (this.img != null) {
            this.s.translate(translateX, translateY)
            this.s.rotate(this.rotation.x)
            this.s.image(this.img, 0, 0, dist(this.vertices[0], this.vertices[1]) * (1000 / this.camera.scale), dist(this.vertices[1], this.vertices[2]) * (1000 / this.camera.scale))
            this.s.rotate(-this.rotation.x)
            this.s.translate(-translateX, -translateY)
